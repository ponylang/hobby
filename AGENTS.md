...
`ssl=` is required (hobby uses `crypto` for cookie signing, and the stdlib `net` package links against OpenSSL/LibreSSL): `3.0.x`, `1.1.x`, or `libressl` (CI uses libressl). Run `make lint` before pushing.
