...
actor Main
  ...
  files from `public/` with custom MIME type mappings
  for `.webp` and `.avif` image formats. These
  extensions are not in the default set, so without
  overrides they would be served as
  ...
  new create(env: Env) =>
    ...
    // Add custom content types for .webp and .avif
