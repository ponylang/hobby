...
actor Main
  ...
  - **Logging**: `after` phase records the request after
    handling completes.
  - **Auth**: `before` phase short-circuits with 401 if
    the token is missing, or stores the authenticated
    user in context data for the handler to read.

  Routes:
  - GET /           -> public, no middleware
  - GET /dashboard  -> protected by AuthMiddleware,
  ...
  new create(env: Env) =>
    ...
    let log_mw: Array[hobby.Middleware val] val =
      recover val
        [as hobby.Middleware val: LogMiddleware(env.out)]
      end
    ...
    hobby.Application
      ...
      .> get(
        "/dashboard",
        DashboardHandler
        where middleware = auth_mw)
      .> get(
        "/health",
        HealthHandler
        where middleware = log_mw)
      .serve(
        auth,
...
primitive HelloHandler is hobby.Handler
  """
  Responds with a static greeting.
  """

  fun apply(ctx: hobby.Context ref) =>
    ctx.respond(
      stallion.StatusOK, "Hello from Hobby!")

primitive HealthHandler is hobby.Handler
  """
  Responds with a simple health-check message.
  """
...
primitive AuthData
  ...
  Middleware authors provide accessor primitives like
  this to give handlers type-safe access to context
  data via `match`, avoiding raw string-key lookups
  and `as` casts.
  """
...
class val AuthMiddleware is hobby.Middleware
  """
  Checks for a `Bearer` token in the Authorization
  header.

  If the token is present, stores an
  `AuthenticatedUser` in context data. If missing,
  short-circuits with 401 -- the handler is never
  invoked, but `after` phases of preceding middleware
  still run.
  """
  ...
  fun before(ctx: hobby.Context ref) =>
    ...
    | let _: String =>
      ...
      ctx.respond(
        stallion.StatusUnauthorized, "Unauthorized")
    end
...
class val LogMiddleware is hobby.Middleware
  ...
  Demonstrates the `after` phase -- it runs regardless
  of whether the handler succeeded, errored, or was
  short-circuited by earlier middleware.
  """
