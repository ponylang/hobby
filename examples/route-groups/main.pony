...
actor Main
  ...
  Demonstrates route grouping with shared prefixes
  and middleware:
  - Application-level middleware (logging) runs on
    every route.
  - A `/api` group with auth middleware protects its
    routes.
  - A nested `/api/admin` group adds admin middleware
    on top of auth.
  - Routes registered directly on the Application have
    no group middleware.
  ...
  new create(env: Env) =>
    ...
    let admin_mw: Array[hobby.Middleware val] val =
      recover val
        [as hobby.Middleware val: AdminMiddleware]
      end
...
primitive UsersHandler is hobby.Handler
  """
  Responds with a placeholder user list.
  """
...
class val UserHandler is hobby.Handler
  """
  Responds with a single user by ID from route params.
  """
...
primitive DashboardHandler is hobby.Handler
  """
  Responds with the admin dashboard page.
  """
