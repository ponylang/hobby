...
// A request stashed during an active streaming response: the HTTP
// request, its Stallion responder, and the accumulated body.
...
actor _Connection is stallion.HTTPServerActor
  ...
  Implements Stallion's `HTTPServerActor` to receive HTTP events.
  Accumulates body chunks, looks up the matching route, and runs
  the middleware chain and handler via `_ChainRunner`.
  """
  var _http: stallion.HTTPServer =
    stallion.HTTPServer.none()
  ...
  // requests arriving during streaming are buffered in
  // _pending_requests and drained when the stream finishes.
  var _streaming_responder: (stallion.Responder | None) =
    None
  ...
  =>
    ...
    let route_match =
      ...
      | let m: _RouteMatch => m
        ...
        // HEAD falls back to GET handler when no explicit HEAD
        // route exists
    ...
    | let m: _RouteMatch =>
      let ctx =
        Context(
          request', responder, m.params, body, this)
  ...
  fun ref _drain_pending() =>
    while
      (_streaming_responder is None)
        and (_pending_requests.size() > 0)
    do
      try
