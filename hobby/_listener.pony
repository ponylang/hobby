...
actor _Listener is lori.TCPListenerActor
  """
  Internal TCP listener that accepts connections and spawns
  connection actors.

  Created by `Application.serve()` with a frozen `_Router val`.
  Each accepted connection gets its own `_Connection` actor.
  """
  ...
  fun ref _on_accept(fd: U32): lori.TCPConnectionActor =>
    _Connection(
      _server_auth, fd, _config, _router)
