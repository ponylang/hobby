...
interface val _ResponseMode
  """
  Strategy for building HTTP responses, separating HEAD from
  standard behavior.

  Standard mode builds full responses with bodies. Head mode builds
  headers-only responses, suppressing bodies while preserving
  Content-Length.
  """
  fun respond(
...
primitive _StandardResponseMode is _ResponseMode
  ...
  =>
    let response =
      stallion.ResponseBuilder(status)
        .add_header(
  ...
  : (StreamSender tag
    ...
    | BodyNotNeeded) ?
  =>
    match responder.start_chunked_response(
      status, headers)
...
primitive _HeadResponseMode is _ResponseMode
  ...
  =>
    ...
    // start_chunked_response(), which would inject
