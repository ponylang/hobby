...
class \nodoc\ iso _TestServeFilesIfModifiedSince304 is UnitTest
  ...
  fun apply(h: TestHelper) =>
    try
      let root = _ServeFilesTestSetup(h.env)?
      let last_mod =
        _ServeFilesTestETag.last_modified(root, "hello.txt")?
      let router =
        _IntegrationHelpers.build_router(
          recover val
            [(stallion.GET, "/static/*filepath", ServeFiles(root), None)]
...
class \nodoc\ iso _TestServeFilesCacheControlCustom is UnitTest
  ...
  fun apply(h: TestHelper) =>
    try
      let root = _ServeFilesTestSetup(h.env)?
      let handler =
        ServeFiles(root where cache_control = "private, max-age=600")
      let router =
        _IntegrationHelpers.build_router(
          recover val
            [(stallion.GET, "/static/*filepath", handler, None)]
...
actor \nodoc\ _TestNoCacheControlClient is
  ...
  """
  TCP client that checks a required string is present and a
  forbidden string is absent.
  """
