...
class iso Application
  ...
  =>
    ...
    """
    _routes.push(
      _RouteDefinition(method, path, handler, middleware))

  fun ref add_middleware(
    middleware: Array[Middleware val] val)
  =>
    """
    Add application-level middleware that runs before every route's
    middleware.

    Can be called multiple times -- middleware accumulates in
    registration order. Application middleware runs before group
    middleware, which runs before per-route middleware.
    """
  ...
  fun ref group(g: RouteGroup iso) =>
    """
    Consume a route group, flattening its routes into this
    application.

    The group's prefix and middleware are applied to each of its
    routes. The group is consumed -- no further registration on it
    is possible.
    """
  ...
  =>
    ...
    Consumes the Application -- no further route registration is
    possible after this call.
    """
    ...
    for r in self._routes.values() do
      let combined_mw =
        _ConcatMiddleware(app_mw, r.middleware)
      builder.add(
        r.method, r.path, r.handler, combined_mw)
    end
